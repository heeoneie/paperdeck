...
**개념 보기** — 제목이 같은 문제를 하나의 개념으로 묶어 보여줍니다.
개념을 열면 그 주제의 기출이 **최근 회차부터** 쌓이고 **답이 모두 드러나 있습니다**
(채점 없음, 읽는 용도). 표의 빈칸도 정답으로 채워진 상태로 나옵니다.
개념 이름 검색과 정렬(많이 나온 순 / 최근 출제순 / 가나다순)을 지원합니다.

현재 데이터 기준 개념 268개 · 문제 388개이며, `수변전 설비 결선도` 36문제,
`감리` 19문제처럼 반복 출제되는 주제가 있는가 하면 1문제뿐인 개념도 237개입니다.
