...
**문제 풀기** — 빈칸에 답을 적고 채점받습니다. 진도가 쌓입니다.

**기출 보기** — 답이 모두 드러난 채로 읽는 화면입니다 (채점 없음).
표의 빈칸도 정답으로 채워진 상태로 나옵니다. 목록은 두 가지로 볼 수 있고
고른 쪽은 `localStorage['pd_bmode']` 에 남습니다.

- **연도별** (기본) — 2026 → 2001 연도 목록. 연도를 열면 **회차별로 묶여** 그 회차에
  나온 순서(책에 실린 페이지 순) 그대로 쌓입니다. 맨 위에 그 해에 나온 **개념 목차**가
  있어 누르면 해당 문제로 바로 갑니다. 다른 해에도 나온 문제는 아래에 `같은 문제:` 로
  표시합니다. 한 해에 두 번 출제된 문제는 두 회차에 모두 들어갑니다
  (그래서 연도 합계가 홈 화면의 연도별 문제 수보다 클 수 있습니다).
- **개념별** — 제목이 같은 문제를 하나로 묶어 **최근 회차부터** 보여줍니다.
  개념 이름 검색과 정렬(많이 나온 순 / 최근 출제순 / 가나다순)을 지원합니다.

현재 데이터 기준 연도 26개 · 회차 78개 · 문제 388개이며, 개념으로 묶으면 268개가 됩니다.
`수변전 설비 결선도` 36문제, `감리` 19문제처럼 반복 출제되는 주제가 있는가 하면
1문제뿐인 개념도 237개라 **개념별 목록은 잘게 쪼개져 보입니다.** 연도별이 기본인 이유입니다.
